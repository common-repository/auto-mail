<?php
/**
 * Auto_Mail_Form_Base_Model Class
 *
 * @since  1.0.0
 * @package Auto Mail
 */
...
if ( ! class_exists( 'Auto_Mail_Form_Base_Model' ) ) :

	abstract class Auto_Mail_Form_Base_Model {

		const META_KEY = 'auto_mail_form';
		 ...
		 * @return mixed
		 */
		public function save( $clone = false, $title = '' ) {

			if( post_exists($title) && is_null($this->id) ){
				return new WP_Error( 'email_exists', sprintf( esc_html__( 'The email "%s" already exists.', Auto_Mail::DOMAIN ), $title ) );
			}
		...
		/**
		 * @since 1.0.0
		 * @return Auto_Mail_Form_Field_Model[]
		 */
		 ...
		 * @param $slug
		 *
		 * @return Auto_Mail_Form_Field|null
		 */
		...
		public function get_all_paged( $current_page = 1, $per_page = null, $status = '' ) {
			if ( is_null( $per_page ) ) {
				$per_page = auto_mail_form_view_per_page();
			}
			$args = array(
				'post_type'      => $this->post_type,
				'post_status'    => 'any',
     ...
     * @return array()
     */
        public function get_all_models( $status = '', $limit = - 1, $keyword = '' ) {
            $args = array(
                'post_type'      => $this->post_type,
                'post_status'    => 'any',
						...
						if ( 'post' === $map['type'] ) {
							...
							if ( ! empty( $meta['fields'] ) && 'fields' === $map['field'] ) {
								foreach ( $meta['fields'] as $field_data ) {
									$field          = new Auto_Mail_Form_Field_Model( $form_settings );
		...
		public function get_fields_grouped() {
			...
			foreach ( $this->fields as $field ) {
				/** @var Auto_Mail_Form_Field_Model $field */
