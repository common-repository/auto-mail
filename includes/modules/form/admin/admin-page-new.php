...
if ( ! class_exists( 'Auto_Mail_Form_New_Page' ) ) :

class Auto_Mail_Form_New_Page extends Auto_Mail_Admin_Page {
    ...
    public function getWizardTitle() {
        if ( isset( $_REQUEST['id'] ) ) { // WPCS: CSRF OK
            return esc_html__( "Edit Form", Auto_Mail::DOMAIN );
        } else {
            return esc_html__( "New Form", Auto_Mail::DOMAIN );
        }
    }
    ...
    public function enqueue_scripts( $hook ) {
        ...
        // Load admin form edit scripts
        auto_mail_admin_enqueue_scripts_form_edit(
            AUTO_MAIL_VERSION,
    ...
    public function get_single_model( $id ) {
        $data = Auto_Mail_Form_Model::model()->get_single_model( $id );
