...
if ( ! class_exists( 'Auto_Mail_Form_Page' ) ) :

class Auto_Mail_Form_Page extends Auto_Mail_Admin_Page {
    ...
    public function enqueue_scripts( $hook ) {
        ...
        auto_mail_admin_enqueue_scripts_form_list(
            AUTO_MAIL_VERSION,
    ...
    public function processRequest() {
        ...
        if ( ! wp_verify_nonce( $nonce, 'auto-mail-form-request' ) ) {
            return;
        }
	...
	public function countModules( $status = '' ) {
		return Auto_Mail_Form_Model::model()->count_all( $status );
	}
    ...
    public function get_models( $limit = null ) {
        $data = Auto_Mail_Form_Model::model()->get_all_paged( $this->page_number, $limit );
    ...
    public function bulk_actions() {
        return apply_filters(
            'auto-mail_form_bulk_actions',
            array(
                'publish-forms'    => esc_html__( "Publish", Auto_Mail::DOMAIN ),
                'draft-forms'      => esc_html__( "Unpublish", Auto_Mail::DOMAIN ),
                'delete-forms'     => esc_html__( "Delete", Auto_Mail::DOMAIN ),
            ) );
    }
    ...
    public function update_module_status( $id, $status ) {
        ...
        if ( in_array( $status, array( 'publish', 'draft' ), true ) ) {
            $model = Auto_Mail_Form_Model::model()->load( $id );
            if ( $model instanceof Auto_Mail_Form_Model ) {
                $model->status = $status;
                $model->save();
            }
        }
    ...
    public function pagination() {
        ...
        auto_mail_list_pagination( $count, 'forms' );
    }
