...
if ( ! class_exists( 'Auto_Mail_Subscriber' ) ) :

class Auto_Mail_Subscriber extends Auto_Mail_Module {
	...
	public function load_admin() {
		if( is_admin() ) {
			...
			new Auto_Mail_Subscriber_Admin();
		}
	}
	...
	public function register_cpt() {
		$labels = array(
			'name'          => $this->get_option('AM_Subscribers'),
			'singular_name' => $this->get_option('am_subscriber')
		);
		...
		register_post_type( 'am_subscribers', $args );
		...
		foreach($custom_status as $key => $value){
			$this->custom_post_status($value, 'am_subscribers');
		}
	}
	 ...
	 * @return array
	 */
	private function custom_post_status($status, $post_type) {
		register_post_status( $status, array(
			'public'                    => true,
			'post_type'                 => array( $post_type ), // Define one or more post types the status can be applied to.
		) );
	}
