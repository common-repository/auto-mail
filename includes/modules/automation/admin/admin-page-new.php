<?php
/**
 * Auto_Mail_Automation_New_Page Class
 *
 * @since  1.0.0
 * @package Auto Mail
 */
...
if ( ! class_exists( 'Auto_Mail_Automation_New_Page' ) ) :

class Auto_Mail_Automation_New_Page extends Auto_Mail_Admin_Page {
    ...
    public function getWizardTitle() {
        if ( isset( $_REQUEST['id'] ) ) { // WPCS: CSRF OK
            return esc_html__( "Edit Automation", Auto_Mail::DOMAIN );
        } else {
            return esc_html__( "New Automation", Auto_Mail::DOMAIN );
        }
    }
    ...
    public function enqueue_scripts( $hook ) {
        ...
        // Load admin automation edit scripts
        auto_mail_admin_enqueue_scripts_automation_edit(
            AUTO_MAIL_VERSION,
    ...
    public function get_single_model( $id ) {
        $data = Auto_Mail_Automation_Model::model()->get_single_model( $id );
