...
if ( ! class_exists( 'Auto_Mail_Campaign_Page' ) ) :

class Auto_Mail_Campaign_Page extends Auto_Mail_Admin_Page {
    ...
    public function enqueue_scripts( $hook ) {
        ...
        auto_mail_admin_enqueue_scripts_campaign_list(
            AUTO_MAIL_VERSION,
    ...
    public function processRequest() {
        ...
        if ( ! wp_verify_nonce( $nonce, 'auto-mail-campaign-request' ) ) {
            return;
        }
        ...
        switch ( $action ) {
                ...
                if ( isset( $ids ) && !empty( $ids ) ) {
                    ...
                    if ( is_array( $campaign_ids ) && count( $campaign_ids ) > 0 ) {
                        foreach ( $campaign_ids as $campaign_id ) {
                            $this->update_module_status( $campaign_id, 'publish' );
                        }
                    }
                }
                break;
            ...
            case 'draft-campaigns' :
                if ( isset( $ids ) && !empty( $ids ) ) {
                    ...
                    if ( is_array( $campaign_ids ) && count( $campaign_ids ) > 0 ) {
                        foreach ( $campaign_ids as $campaign_id ) {
                            $this->update_module_status( $campaign_id, 'draft' );
                        }
                    }
                }
                break;
	...
	public function countModules( $status = '' ) {
		return Auto_Mail_Campaign_Model::model()->count_all( $status );
	}
    ...
    public function get_models( $limit = null ) {
        $data = Auto_Mail_Campaign_Model::model()->get_all_paged( $this->page_number, $limit );
    ...
    public function bulk_actions() {
        return apply_filters(
            'auto-mail_campaign_bulk_actions',
            array(
                'publish-campaigns'    => esc_html__( "Publish", Auto_Mail::DOMAIN ),
                'draft-campaigns'      => esc_html__( "Unpublish", Auto_Mail::DOMAIN ),
                'delete-campaigns'     => esc_html__( "Delete", Auto_Mail::DOMAIN ),
            ) );
    }
    ...
    public function update_module_status( $id, $status ) {
        ...
        if ( in_array( $status, array( 'publish', 'draft' ), true ) ) {
            $model = Auto_Mail_Campaign_Model::model()->load( $id );
            if ( $model instanceof Auto_Mail_Campaign_Model ) {
                $model->status = $status;
                $model->save();
            }
        }
    ...
    public function pagination() {
        ...
        auto_mail_list_pagination( $count, 'campaigns' );
    }
