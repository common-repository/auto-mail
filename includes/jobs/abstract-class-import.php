...
if ( ! class_exists( 'Auto_Mail_Import' ) ) :

abstract class Auto_Mail_Import {
	...
	protected function update_credentials( $data, $expiration = null ) {
		...
		set_transient( 'auto_mail_importer_credentials_' . $this->type, $data, $expiration );
		$this->credentials = $data;
	}

	protected function get_credentials() {
		if ( ! $this->credentials ) {
			$this->credentials = get_transient( 'auto_mail_importer_credentials_' . $this->type );
		}

		return $this->credentials;
	}

	public function valid_credentials($data) {
		return true;
	}

	abstract protected function get_import_part( &$import_data );
	abstract protected function get_import_data( $paste_data );
	...
	public function import_options( $data = null ) {
		...
		if ( method_exists( $this, 'credentials_form' ) && ! $this->get_credentials() ) :
			...
			<section class="footer alternate">
				...
				<p><?php submit_button( __( 'Next Step', Auto_Mail::DOMAIN ) . '  &#x2192;', 'primary', 'submit', false ); ?></p>
			</section>
			<?php
		else :
			...
			?>
			<p><strong><?php esc_html_e( 'Please select the lists you like to import.', Auto_Mail::DOMAIN ); ?></strong></p>
			<ul>
			...
			<p><strong><?php esc_html_e( 'Please select the statuses you like to import.', Auto_Mail::DOMAIN ); ?></strong></p>
			<ul>
